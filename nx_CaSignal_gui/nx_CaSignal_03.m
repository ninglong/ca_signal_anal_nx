function varargout = nx_CaSignal_03(varargin)
% NX_CASIGNAL_03 M-file for nx_CaSignal_03.fig
%      NX_CASIGNAL_03, by itself, creates a new NX_CASIGNAL_03 or raises the existing
%      singleton*.
%
%      H = NX_CASIGNAL_03 returns the handle to a new NX_CASIGNAL_03 or the handle to
%      the existing singleton*.
%
%      NX_CASIGNAL_03('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NX_CASIGNAL_03.M with the given input arguments.
%
%      NX_CASIGNAL_03('Property','Value',...) creates a new NX_CASIGNAL_03 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before nx_CaSignal_03_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to nx_CaSignal_03_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help nx_CaSignal_03

% Last Modified by GUIDE v2.5 11-Jan-2011 11:50:07
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @nx_CaSignal_03_OpeningFcn, ...
                   'gui_OutputFcn',  @nx_CaSignal_03_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before nx_CaSignal_03 is made visible.
function nx_CaSignal_03_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for nx_CaSignal_03
...
handles.userdata.h_info_fig = NaN;
handles.userdata.FrameNum = 1;
handles.userdata.imSize = [];
handles.userdata.h_img = NaN;
handles.userdata.Scale = [0 500];
ROIinfo = {};
ICA_ROIs = struct;
handles.userdata.ROIplot = NaN;
...
handles.userdata.ICA_figs = nan(1,2);
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes nx_CaSignal_03 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = nx_CaSignal_03_OutputFcn(hObject, eventdata, handles) 
...
global CaTrials ROIinfo ICA_ROIs
...
handles.userdata.imSize = size(im);
if ~isempty(CaTrials)
...
if get(handles.dispModeWithROI,'Value') == 1
    ...
    if any(ishandle(handles.userdata.ROIplot))
        try
            delete(handles.userdata.ROIplot(ishandle(handles.userdata.ROIplot)));
        end
    end
    handles.userdata.ROIplot = plot_ROIs(handles);
end
...
%             end
            ...
            text(median(roi_pos{i}(:,1)), median(roi_pos{i}(:,2)), num2str(i),'Color','w','FontSize',15);
...
if get(handles.ICA_ROI_anal,'Value') == 1
    ICA_ROIs.ROIpos{CurrentROINo} = pos;
    ICA_ROIs.ROIMask{CurrentROINo} = BW;
    ICA_ROIs.ROIType{CurrentROINo} = ROIType;
    set(handles.nROIsText,'String', num2str(length(ICA_ROIs.ROIpos)));
else
    ...
    ROIinfo{TrialNo}.ROIType{CurrentROINo} = ROIType;
    
end
axes(handles.Image_disp_axes);
delete(h_poly); % delete polygon object

if length(handles.userdata.ROIplot) >= CurrentROINo
...
if get(handles.ICA_ROI_anal, 'Value') == 1
    ...
    ROImask = ROIinfo{TrialNo}.ROIMask;
end
if nargin < 4
im = handles.userdata.ImageArray;
end    
...
if get(handles.ICA_ROI_anal, 'Value') == 1
    for i = 1:length(CaTrials)
        CaTrials(i).nROIs = length(ICA_ROIs.ROIpos);
        CaTrials(i).ROIinfo.ROIpos = ICA_ROIs.ROIpos;
        CaTrials(i).ROIinfo.ROImask = ICA_ROIs.ROImask;
        CaTrials(i).ROIinfo.ROItype = ICA_ROIs.ROItype;
    end
else
for i = 1:length(ROIinfo)
    if isfield(ROIinfo{i},'ROIpos')
        CaTrials(i).nROIs = length(ROIinfo{i}.ROIpos);
        CaTrials(i).ROIinfo.ROIpos = ROIinfo{i}.ROIpos;
        CaTrials(i).ROIinfo.ROImask = ROIinfo{i}.ROImask;
        CaTrials(i).ROIinfo.ROItype = ROIinfo{i}.ROItype;
    end
end
end
...
if get(hObject, 'Value') == 1
    % load_saved_data_SVD
    if isempty(handles.userdata.ImageArray)
    ...
    handles.userdata.ica_data = ica_data;
    handles.userdata.currentIC = 1;
...
if get(handles.ICA_ROI_anal,'Value') == 1 && handles.userdata.currentIC > 1
    handles.userdata.currentIC = handles.userdata.currentIC - 1;
    set(handles.current_ICnum_text, 'String', num2str(handles.userdata.currentIC));
    guidata(hObject, handles);
    handles = disp_ICA(handles);;
end


% --- Executes on button press in nextIC_button.
function nextIC_button_Callback(hObject, eventdata, handles)
if get(handles.ICA_ROI_anal,'Value') == 1 && handles.userdata.currentIC < size(handles.userdata.ICA_components,1)
    handles.userdata.currentIC = handles.userdata.currentIC + 1;
    set(handles.current_ICnum_text, 'String', num2str(handles.userdata.currentIC));
    guidata(hObject, handles);
    handles = disp_ICA(handles);
...
data = handles.userdata.ica_data.Data;
V = handles.userdata.ica_data.V;
S = handles.userdata.ica_data.S;
ICnum = str2num(get(handles.IC_num_edit,'String'));
% handles.userdata.ICnum = str2num(get(handles.IC_num_edit,'String'));
handles.userdata.ICA_components = run_ICA(handles.userdata.ica_data.Data, {S, V, 30, ICnum});
% handles.userdata.ICnum_prev = ICnum;
guidata(handles.figure1, handles);
handles = disp_ICA(handles);
...
% hObject    handle to IC_num_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function handles = disp_ICA(handles)
RowNum = handles.userdata.imSize(1);
ColNum = handles.userdata.imSize(2);
if ~ishandle(handles.userdata.ICA_figs)
    handles.userdata.ICA_figs(1) = figure('Position', [123   460   512   512]);
    handles.userdata.ICA_figs(2) = figure('Position',[115    28   512   512]);
end
disp_ICAcomponent_and_blobs(handles.userdata.ICA_components(handles.userdata.currentIC,:),RowNum, ColNum, handles.userdata.ICA_figs);
for i = 1:length(handles.userdata.ICA_figs)
    figure(handles.userdata.ICA_figs(i)),
    plot_ROIs(handles);
    title(sprintf('IC #%d',handles.userdata.currentIC),'FontSize',15);
end
guidata(handles.figure1, handles);
