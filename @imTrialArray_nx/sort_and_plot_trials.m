...
function h_figs = sort_and_plot_trials(obj, ROInum, sorting,color_scale)
...
%% Sort trials by Answer Lick time
...
if exist('sorting','var') && strcmpi(sorting,'AnswerLick')
    ...
    plot(ts, miss_mean, 'm', 'LineWidth', 1.5);
    plot(ts, cr_mean,'g', 'LineWidth', 1.5);
    plot(ts, fa_mean,'r', 'LineWidth', 1.5);
    ...
    go_mean = mean(tr_go.get_CaTraces(ROInum,'asis'),1);
    go_se = std(tr_go.get_CaTraces(ROInum,'asis'),0,1)./sqrt(length(tr_go));
    nogo_mean = mean(tr_nogo.get_CaTraces(ROInum,'asis'),1);
    nogo_se = std(tr_nogo.get_CaTraces(ROInum,'asis'),0,1)./sqrt(length(tr_nogo));
...
polePos_colors = color_go_pos(tr);
