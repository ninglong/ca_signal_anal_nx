...
% Last Modified by GUIDE v2.5 10-Mar-2012 15:59:59
...
%     set(gca,'visible','off'); hold on;
    ...
    CaImage.imDispType = 'raw_data';
    disp_image(handles)
    
    
function [] = disp_image(handles, imDataType)

global CaImage ROIinfo
figure(CaImage.hfig); cla;
clim = CaImage.clim;
% text()
switch CaImage.imDispType
    case 'raw_data'
        frNo = CaImage.frNo;
        imagesc(CaImage.data(:,:,frNo),clim); colormap(gray);
    case 'max_delta'
        trNo = CaImage.trialNo;
        frNo = trNo;
        imagesc(CaImage.imMaxDeltaArray(:,:,trNo),clim); colormap(gray);
end
...
% --- Executes on button press in load_maxDelta_array.
function load_maxDelta_array_Callback(hObject, eventdata, handles)
% hObject    handle to load_maxDelta_array (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global CaImage ROIinfo
[fn_maxdeltaarray, pathname] = uigetfile;
load(fullfile(pathname, fn_maxdeltaarray), '-mat');
CaImage.imMaxDeltaArray = im_maxDelta_array;
CaImage.imDispType = 'max_delta';
disp_image(handles);
...
switch CaImage.imDispType
    case 'raw_data'
        im_array = CaImage.data;
    case 'max_delta'
        im_array = CaImage.imMaxDeltaArray;
end
...
set(handles.frame_num, 'String', num2str(CaImage.frNo));
if strcmp(CaImage.imDispType, 'max_delta')
    set(handles.trialNo_edit, 'String', num2str(CaImage.frNo));
end
...
function results_save_dir_Callback(hObject, eventdata, handles)
global CaImage
CaImage.results_save_dir = get(hObject, 'String');


% --- Executes during object creation, after setting all properties.
function results_save_dir_CreateFcn(hObject, eventdata, handles)
% hObject    handle to results_save_dir (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
