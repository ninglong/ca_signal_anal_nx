...
% Last Modified by GUIDE v2.5 14-May-2009 09:10:02
...
if ~isfield(handles.userdata, 'ROIPoly')
    handles.userdata.ROIPoly = {}; % ROI polygon votex vectors
    handles.userdata.h_poly = []; % handles to ROI polygon drawings
elseif ~isempty(handles.userdata.ROIPoly)
    %if isempty(handles.userdata.h_poly) || length(handles.userdata.ROIPoly)>length(handles.userdata.h_poly)
    for i = 1:length(handles.userdata.ROIPoly)
        xi = handles.userdata.ROIPoly{i}(:,1);
        yi = handles.userdata.ROIPoly{i}(:,2);
