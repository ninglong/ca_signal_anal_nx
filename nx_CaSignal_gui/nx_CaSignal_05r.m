function varargout = nx_CaSignal_05(varargin)
% NX_CASIGNAL_05 M-file for nx_CaSignal_05.fig
%      NX_CASIGNAL_05, by itself, creates a new NX_CASIGNAL_05 or raises the existing
%      singleton*.
%
%      H = NX_CASIGNAL_05 returns the handle to a new NX_CASIGNAL_05 or the handle to
%      the existing singleton*.
%
%      NX_CASIGNAL_05('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NX_CASIGNAL_05.M with the given input arguments.
%
%      NX_CASIGNAL_05('Property','Value',...) creates a new NX_CASIGNAL_05 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before nx_CaSignal_05_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to nx_CaSignal_05_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help nx_CaSignal_05

% Last Modified by GUIDE v2.5 02-Feb-2012 14:11:18
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @nx_CaSignal_05_OpeningFcn, ...
                   'gui_OutputFcn',  @nx_CaSignal_05_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before nx_CaSignal_05 is made visible.
function nx_CaSignal_05_OpeningFcn(hObject, eventdata, handles, varargin)
global CaSignal % ROIinfo ICA_ROIs
% Choose default command line output for nx_CaSignal_05
...
% UIWAIT makes nx_CaSignal_05 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = nx_CaSignal_05_OutputFcn(hObject, eventdata, handles) 
...
[im, header] = imread_multi(filename, 'r');
% t_elapsed = toc;
...
function import_ROIinfo(ROIinfo, handles) % getROIinfoButton_Callback(hObject, eventdata, handles)
...
%     disp(['Batch analyzing ' num2str(TrialNo) ' of total ' num2str(End_trial-Start_trial+1) ' trials...']);
    ...
    [im, header] = imread_multi(fname, 'r');
    if (length(CaSignal.CaTrials)<TrialNo || isempty(CaSignal.CaTrials(TrialNo).FileName))
    ...
    case 3 % Compute cross correlation across all trials
        ...
        if isempty(CaSignal.avgCorrCoef_trials)
            ...
            for i = 1:n_trials
                ...
                img = imread_multi(CaSignal.data_file_names{i},'r');
                xcoef = xcoef_img(img);
...
global CaSignal
if get(hObject,'Value') == 1
%     [fn, pth] = uigetfile('*.tif','Choose maxDelta Image');
