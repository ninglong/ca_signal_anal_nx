...
CaSignal.results_fn = [CaSignal.results_path filesep 'CaTrials_' FileName_prefix '.mat'];
...
if plot_flag == 1
    CaSignal.h_CaTrace_fig = plot_CaTraces_ROIs(dff, ts);
end
guidata(handles.figure1, handles);
...
if get(handles.ICA_ROI_anal, 'Value') == 1
    for i = 1:length(CaSignal.CaTrials)
        CaSignal.CaTrials(i).nROIs = length(ICA_ROIs.ROIpos);
        CaSignal.CaTrials(i).ROIinfo.ROIpos = ICA_ROIs.ROIpos;
        CaSignal.CaTrials(i).ROIinfo.ROImask = ICA_ROIs.ROImask;
        CaSignal.CaTrials(i).ROIinfo.ROItype = ICA_ROIs.ROItype;
    end
else
for i = 1:length(ROIinfo)
    if isfield(ROIinfo{i},'ROIpos')
        CaSignal.CaTrials(i).nROIs = length(ROIinfo{i}.ROIpos);
        CaSignal.CaTrials(i).ROIinfo.ROIpos = ROIinfo{i}.ROIpos;
        CaSignal.CaTrials(i).ROIinfo.ROImask = ROIinfo{i}.ROImask;
        CaSignal.CaTrials(i).ROIinfo.ROItype = ROIinfo{i}.ROItype;
    end
end
end
CaTrials = CaSignal.CaTrials;
save(CaSignal.results_fn, 'CaTrials');
...
global CaSignal ROIinfo ICA_ROIs
save_gui_info(handles);
clear CaSignal.CaTrials ROIinfo
% close all;
...
function dFF_array = get_dFF_roi(CaSignal.CaTrials, roiNo)
